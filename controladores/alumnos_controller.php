...
switch ($_GET["op"]) {
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
            "0"=>($reg->is_active)?'<button class="btn btn-primary btn-xs" onclick="mostrar('.$reg->id.')"><i class="fa fa-pencil-square-o"></i></button>'.' '.'<button class="btn btn-danger btn-xs" onclick="desactivar('.$reg->id.')"><i class="fa fa-eraser"></i></button>':'<button class="btn btn-secondary btn-xs" onclick="mostrar('.$reg->id.')"><i class="fa fa-pencil-square-o"></i></button>'.' '.'<button class="btn btn-primary btn-xs" onclick="activar('.$reg->id.')"><i class="fa fa-check"></i></button>',
